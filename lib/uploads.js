...
// Restaurant submission media: one required cover photo (field 'image') plus
// one or more menu photos (field 'menu_image'). Exposed as req.files.
const uploadRestaurantMedia = multer({
  storage,
  fileFilter,
  limits: { fileSize: 5 * 1024 * 1024, files: 6 },
}).fields([
  { name: 'image', maxCount: 1 },
  { name: 'menu_image', maxCount: 5 },
]);
...
async function storeUpload(file) {
  ...
  let prefix = 'restaurants';
  if (file.fieldname === 'avatar') prefix = 'avatars';
  else if (file.fieldname === 'menu_image') prefix = 'menus';
...
module.exports = { uploadImage, uploadRestaurantMedia, storeUpload, removeStoredObject, resolveStoredUrl };